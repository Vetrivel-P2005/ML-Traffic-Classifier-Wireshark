...
# Global classifier variable for Jupyter notebook usage
classifier = None
...
def main():
    ...
    pcap_folder = r"E:\pcap"  # Replace with your folder path
...
# Test functions for Jupyter notebook usage
def classify_packet_examples(trained_classifier):
    """Examples of classifying different types of packets"""
    if trained_classifier is None:
        print("Error: Classifier not trained. Please run main() first.")
        return
    
    from scapy.all import IP, TCP, UDP, ICMP
    
    print("\n" + "="*50)
    print("TESTING PACKET CLASSIFICATION")
    print("="*50)
    
    # Example 1: HTTP packet
    http_packet = IP(dst="192.168.1.1")/TCP(dport=80)
    pred_class, confidence = trained_classifier.predict_packet_class(http_packet)
    print(f"HTTP packet: {pred_class} (confidence: {confidence:.3f})")
    
    # Example 2: DNS packet  
    dns_packet = IP(dst="8.8.8.8")/UDP(dport=53)
    pred_class, confidence = trained_classifier.predict_packet_class(dns_packet)
    ...
    pred_class, confidence = trained_classifier.predict_packet_class(https_packet)
    print(f"HTTPS packet: {pred_class} (confidence: {confidence:.3f})")
    
    # Example 4: ICMP packet
    icmp_packet = IP(dst="192.168.1.1")/ICMP()
    pred_class, confidence = trained_classifier.predict_packet_class(icmp_packet)
    print(f"ICMP packet: {pred_class} (confidence: {confidence:.3f})")
    
    # Example 5: SSH packet
    ssh_packet = IP(dst="192.168.1.100")/TCP(dport=22)
    pred_class, confidence = trained_classifier.predict_packet_class(ssh_packet)
    print(f"SSH packet: {pred_class} (confidence: {confidence:.3f})")
    
    # Example 6: FTP packet
    ftp_packet = IP(dst="ftp.example.com")/TCP(dport=21)
    pred_class, confidence = trained_classifier.predict_packet_class(ftp_packet)
    print(f"FTP packet: {pred_class} (confidence: {confidence:.3f})")

def classify_new_pcap_file(trained_classifier, pcap_file_path):
    ...
    try:
        ...
        results = []
        for i, packet in enumerate(packets):
            predicted_class, confidence = trained_classifier.predict_packet_class(packet)
            results.append({
                'packet_id': i,
